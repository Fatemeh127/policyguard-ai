...
limiter = Limiter(
    key_func=get_identifier, storage_uri=settings.redis_url, strategy="fixed-window", enabled=True
)
...
def custom_rate_limit_exceeded_handler(request: Request, exc: RateLimitExceeded):
    ...
    """
    logger.warning(
        "Rate limit exceeded | identifier=%s | path=%s", get_identifier(request), request.url.path
    )
